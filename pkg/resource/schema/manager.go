...
// +kubebuilder:rbac:groups=glue.services.k8s.aws,resources=schemas,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=glue.services.k8s.aws,resources=schemas/status,verbs=get;update;patch

var lateInitializeFieldNames = []string{}

// resourceManager is responsible for providing a consistent way to perform
// CRUD operations in a backend AWS service API for Schema custom resources.
...
// Note: Schema ARNs require both registry name and schema name:
// arn:aws:glue:{region}:{account}:schema/{registry-name}/{schema-name}
func (rm *resourceManager) ARNFromName(name string) string {
	// For Schema, we need the registry name as well, which should be available
	// in the Status.RegistryARN after reference resolution.
	// This method is primarily used for new resources, so we cannot construct
	// a full ARN without the registry information.
	// The actual ARN will be set during creation when we have the registry context.
	return fmt.Sprintf(
		"arn:aws:glue:%s:%s:schema/%s",
		rm.awsRegion,
		rm.awsAccountID,
		name,
	)
...
// FilterSystemTags ignores tags that have keys that start with "aws:"
