...
type resourceManagerFactory struct {
	sync.RWMutex
	// rmCache contains resource managers, keyed by the AWS account ID, AWS
	// region and roleARN that the manager is configured for
	rmCache map[string]*resourceManager
}

// ResourceDescriptor returns an AWSResourceDescriptor object that describes
// the AWS resource managed by this manager.
...
// supplied AWS account and region.
...
// IsAdoptable returns true if the resource is adoptable
func (f *resourceManagerFactory) IsAdoptable() bool {
	return true
}

// RequeueOnSuccessSeconds returns the number of seconds to requeue after a
// successful reconciliation
func (f *resourceManagerFactory) RequeueOnSuccessSeconds() int {
	return 0
}

// newResourceManagerFactory returns a new resourceManagerFactory object
