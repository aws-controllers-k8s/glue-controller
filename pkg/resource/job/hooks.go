...
// jobARN returns the ARN of the Glue job with the given name.
func jobARN(job *svcapitypes.Job) string {
	...
	// the `aws` partition.
	return fmt.Sprintf("arn:aws:glue:%s:%s:job/%s", *job.Status.ACKResourceMetadata.Region, *job.Status.ACKResourceMetadata.OwnerAccountID, *job.Spec.Name)
}
